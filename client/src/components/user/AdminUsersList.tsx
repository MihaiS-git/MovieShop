...
import UserListFiltersBlock from "./UserListFiltersBlock";
import AdminMobileUserCard from "./AdminMobileUserCard";
import AdminUserCard from "./AdminUserCard";

type Props = {
  userPagination: ItemsPagination;
  userFilters: UserFilters;
  users: UserItem[];
  handleDelete: (id: number) => void;
};

const AdminUsersList: React.FC<Props> = ({
  userPagination,
  userFilters,
  users,
  handleDelete,
  ...
  const { hasMore, loadMore, totalPages } = userPagination;
  ...
  return (
    <>
      ...
      {users?.length === 0 ? (
        <p className="h-screen pt-24 text-2xl text-center w-full">
        ...
        <>
            ...
            {users?.map((user) => (
              <li key={user.id}>
                {isMobile ? (
                  <AdminMobileUserCard
                    user={user}
                    handleDeleteClick={() => handleDelete(user.id)}
                  />
                ) : (
                  <AdminUserCard
                    user={user}
          ...
          {isMobile ? (
            ...
            totalPages > 1 && (
              <ListItemsPagination listItemsPagination={userPagination} />
            )
          )}
        </>
      )}
